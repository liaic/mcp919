...
public class TextureUtils {
   ...
   public static void applyAnisotropicLevel() {
      if(GLContext.getCapabilities().GL_EXT_texture_filter_anisotropic) {
         float maxLevel = GL11.glGetFloat('\u84ff');
         float level = (float)Config.getAnisotropicFilterLevel();
         level = Math.min(level, maxLevel);
         GL11.glTexParameterf(3553, '\u84fe', level);
      }
   ...
   public static void dbgMipmaps(TextureAtlasSprite textureatlassprite) {
      int[][] data = textureatlassprite.getFrameTextureData(0);
   ...
   public static void saveGlTexture(String name, int textureId, int mipmapLevels, int width, int height) {
      ...
      for(level = 0; level <= mipmapLevels; ++level) {
         ...
         GL11.glGetTexImage(3553, level, '\u80e1', '\u8367', buf);
         buf.get(data);
   ...
   public static int getGLMaximumTextureSize() {
      for(int i = 65536; i > 0; i >>= 1) {
         GL11.glTexImage2D('\u8064', 0, 6408, i, i, 0, 6408, 5121, (IntBuffer)null);
         int err = GL11.glGetError();
         int width = GL11.glGetTexLevelParameteri('\u8064', 0, 4096);
         if(width != 0) {
            return i;
         }
      }
